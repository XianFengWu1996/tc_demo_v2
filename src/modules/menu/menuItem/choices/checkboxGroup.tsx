...
export const CheckboxGroup = (props: ICheckboxGroup) => {
  ...
  useEffect(() => {
    ...
    const found_selected_choice = selectedChoices.find((ch) => {
  ...
  const handleCheckboxOnChange = (checked: boolean, option: IOption) => {
    ...
    if (checked) {
      ...
      // removing
      const found_index = selectedOptions.findIndex((opt) => {
...
export const CheckboxItem = (props: ICheckboxItem) => {
  ...
  return (
    <FormControlLabel
      ...
      label={`${option.en_option} ${option.ch_option} ${
